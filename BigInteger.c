...
struct BigInteger mul(struct BigInteger num1, struct BigInteger num2) {
    ...
    struct BigInteger num3 = initialize("0"); // Initialize with "0"
    num3.length = 0;
    ...
    struct BigInteger mid, result = initialize("0"); // Initialize with "0"
    int carry = 0, product = 0, i = 0;

    while (n2 != NULL) {
        n1 = num1.head;
        carry = 0;
        mid = initialize(""); // Initialize with an empty string
        for (int j = 0; j < i; j++) {
            insert_tail(&mid, 0);
        }
        while (n1 != NULL) {
